...
import { ShieldCheck, CheckCircle2, ArrowRight, Award, Building2 } from "lucide-react";
...
export default function ExperiencePage() {
  ...
  return (
    <div className="bg-slate-50 dark:bg-[#0A0D0C] text-slate-900 dark:text-white overflow-x-hidden min-h-screen transition-colors duration-300">
        ...
        customVisual={
          <div className="bg-white dark:bg-white/[0.03] text-slate-900 dark:text-white p-8 sm:p-10 rounded-3xl border border-slate-200 dark:border-white/10 shadow-2xl space-y-6 backdrop-blur-2xl relative group hover-green-box transition-all duration-500">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-white/10 pb-4">
              <span className="text-xs font-mono text-slate-500 dark:text-[#D8CCB8] uppercase tracking-wider">TRACK RECORD DASHBOARD</span>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-4 bg-slate-50 dark:bg-white/[0.03] border-l-4 border-[#10E784] rounded-2xl">
                <span className="text-3xl font-display font-bold text-[#10E784]">50+</span>
                <span className="text-[11px] text-slate-600 dark:text-[#B9B3A8] block mt-1">Enterprise Projects</span>
              </div>
              <div className="p-4 bg-slate-50 dark:bg-white/[0.03] border-l-4 border-[#10E784] rounded-2xl">
                <span className="text-3xl font-display font-bold text-[#10E784]">SR 100M+</span>
                <span className="text-[11px] text-slate-600 dark:text-[#B9B3A8] block mt-1">Delivered Value</span>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-50 dark:bg-white/[0.03] rounded-2xl flex items-center gap-3">
              ...
              <div>
                <div className="text-xs font-bold text-slate-900 dark:text-white">Historical Partner: SLNEE</div>
                <div className="text-[10px] text-slate-500 dark:text-[#B9B3A8]">Government & Semi-Government Implementations</div>
              </div>
            </div>
          </div>
        }
      ...
      <section className="bg-white dark:bg-[#101312] py-12 border-b border-slate-200 dark:border-white/10 transition-colors duration-300">
        <div className="max-w-[1280px] mx-auto px-6 lg:px-8">
          <div className="p-8 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-xl backdrop-blur-xl">
            <div className="flex items-start gap-4">
              <ShieldCheck className="h-8 w-8 text-[#10E784] shrink-0 mt-1" />
              <div>
                <span className="text-xs font-mono font-bold text-[#10E784] tracking-widest uppercase block mb-1">
                ...
                <p className="text-xs text-slate-600 dark:text-[#B9B3A8] font-light max-w-2xl leading-relaxed">
      ...
      <section className="py-16 bg-slate-50 dark:bg-[#0A0D0C] border-b border-slate-200 dark:border-white/10 transition-colors duration-300">
        <div className="max-w-[1280px] mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {KEY_CREDIBILITY_METRICS.map((metric, idx) => (
              <div key={idx} className="p-8 bg-white dark:bg-white/[0.03] rounded-3xl text-center shadow-md border border-slate-200 dark:border-white/5 hover:border-[#10E784]/60 transition-all backdrop-blur-md">
                ...
                <div className="text-xs text-slate-500 dark:text-[#B9B3A8] font-light">{metric.subtext}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-[var(--space-section-lg)] bg-white dark:bg-[#101312] transition-colors duration-300">
        <div className="max-w-[1280px] mx-auto px-6 lg:px-8">
          <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <p className="text-[#10E784] text-xs font-mono font-bold tracking-widest mb-3 uppercase">
                CASE STUDY INDEX
              </p>
              <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 dark:text-white">
          ...
          <div className={filteredItems.length === 1 ? "flex justify-center" : "grid grid-cols-1 md:grid-cols-2 gap-8"}>
            {filteredItems.map((item, idx) => (
              <motion.div
                ...
                className={`bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10 rounded-3xl p-8 sm:p-10 flex flex-col justify-between hover-green-box transition-all duration-300 shadow-xl group backdrop-blur-xl ${
                  filteredItems.length === 1 ? "w-full max-w-2xl" : ""
                }`}
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-xs font-mono font-bold text-[#10E784] uppercase tracking-wider px-3.5 py-1 bg-[#10E784]/10 border border-[#10E784]/30 rounded-full">
                      {item.clientSector} SECTOR
                    </span>
                    <span className="text-xs font-bold text-slate-500 dark:text-[#B9B3A8]">
                      {item.deliveryPartner}
                    </span>
                  </div>

                  <h3 className="font-display text-2xl text-slate-900 dark:text-white font-bold mb-4 group-hover:text-[#10E784] transition-colors">
                    {item.title}
                  </h3>

                  <p className="text-slate-600 dark:text-[#B9B3A8] text-sm leading-relaxed mb-6 font-light">
                    {item.summary}
                  </p>

                  <div className="space-y-3 mb-6 bg-white dark:bg-white/[0.03] p-5 rounded-2xl border-l-4 border-[#10E784] shadow-sm">
                    <div className="text-xs font-mono font-bold uppercase tracking-wider text-slate-700 dark:text-[#D8CCB8] mb-2">
                      Verified Deliverables & Impact:
                    </div>
                    {item.impactMetrics.map((m, i) => (
                      <div key={i} className="flex items-center gap-3 text-xs text-slate-600 dark:text-[#B9B3A8]">
                        <CheckCircle2 className="h-4 w-4 text-[#10E784] shrink-0" />
                        <span className="font-bold text-slate-900 dark:text-white">{m}</span>
                      </div>
                    ))}
                  </div>
                </div>

                <div className="pt-4 border-t border-slate-200 dark:border-white/10 flex items-center gap-2 flex-wrap text-[11px] font-medium text-slate-600 dark:text-[#B9B3A8]">
                  {item.capabilitiesUsed.map((c, i) => (
                    <span key={i} className="px-3 py-1 bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 rounded-full text-slate-700 dark:text-[#D8CCB8] group-hover:border-[#10E784]/40 transition-colors">
                      {c}
                    </span>
                  ))}
                </div>
